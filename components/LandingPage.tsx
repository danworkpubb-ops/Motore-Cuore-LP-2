...
interface LandingPageProps {
  ...
  onUpdateContent?: (newContent: GeneratedContent) => void; // NEW: Callback for inline editing
  onGoHome?: () => void;
}
...
const DEFAULT_LABELS = {
    ...
    cardErrorMsg: "Errore",
    switchToCod: "Cambia",
    mostPopular: "La più scelta",
    mostRequested: "Più richiesta",
    giveUpOffer: "Annulla",
    confirmCod: "Conferma",
...
const OrderPopup: React.FC<{ 
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (content.variants?.enabled && content.variants.required && !selectedVariant) {
            setVariantError?.(true);
            
            // Scroll to variants section if it exists in popup
            const popupScroll = document.querySelector('.custom-scrollbar');
            const variantEl = document.getElementById('popup-variants-section');
            if (popupScroll && variantEl) {
                popupScroll.scrollTo({ top: variantEl.offsetTop - 20, behavior: 'smooth' });
            }
                    ...
                    {content.formType === 'html' ? (
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            {content.variants?.enabled && content.variants.options.length > 0 && (
                                <div className="space-y-3 mb-6 animate-in fade-in slide-in-from-top-2" id="popup-variants-section">
                                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider">
                                        {content.variants.title || 'Scegli la variante:'}
                                        {content.variants.required && <span className="text-red-500 ml-1">*</span>}
                                    </label>
                                    <div className="grid grid-cols-1 gap-2">
                                        {content.variants.options.map((option) => (
                                            <label 
                                                key={option.id} 
                                                className={`flex items-center p-3 border rounded-xl cursor-pointer transition-all relative ${selectedVariant === option.id ? 'border-slate-900 bg-slate-50 ring-1 ring-slate-900' : 'border-slate-200 hover:border-slate-300'}`}
                                            >
                                                {option.isMostPopular && (
                                                    <div className="absolute -top-2 -right-1 bg-slate-900 text-white text-[8px] font-black px-1.5 py-0.5 rounded uppercase tracking-wider shadow-sm z-10 animate-pulse">
                                                        {labels.mostPopular}
                                                    </div>
                                                )}
                                                <input 
                            ...
                            <div className="pt-2">
                                {variantError && (
                                    <div className="bg-red-50 border border-red-200 text-red-600 p-3 rounded-lg text-xs font-bold flex items-center gap-2 animate-in fade-in slide-in-from-top-1 mb-3">
                                        <AlertTriangle className="w-4 h-4 shrink-0" />
                                        <span>{content.language === 'Italiano' || !content.language ? `Seleziona "${content.variants?.title || 'una variante'}" per continuare` : `Select "${content.variants?.title || 'a variant'}" to continue`}</span>
                                    </div>
                                )}
...
const GadgetTemplate: React.FC<TemplateProps> = ({ content, onBuy, styles, siteConfig, selectedVariant, setSelectedVariant, selectedSubVariants, setSelectedSubVariants, variantError, onUpdateContent, onGoHome }) => {
    ...
    return (
        <div className={`min-h-screen ${!content.backgroundColor ? 'bg-slate-50' : ''} text-slate-800 pb-32 md:pb-0 selection:bg-blue-100 selection:text-blue-900 ${styles.font}`} style={{...bodyStyle, ...backgroundStyle}}>
            ...
            {/* PULSANTE TORNA ALLA HOME - SOLO DESKTOP */}
            <a 
                href="/" 
                onClick={(e) => {
                    if (onGoHome) {
                ...
                className="hidden md:flex fixed top-12 left-6 z-[40] items-center gap-2 bg-white/80 backdrop-blur-md border border-slate-200 py-2.5 px-4 rounded-full shadow-lg hover:bg-white hover:shadow-xl transition-all group animate-in fade-in slide-in-from-left-4 duration-500"
            >
                <ArrowLeft className="w-4 h-4 text-slate-600 group-hover:-translate-x-1 transition-transform" />
                <span className="text-xs font-bold text-slate-700 uppercase tracking-wider">Torna alla Home</span>
            </a>
                    ...
                    <div className="w-full order-2 px-5 md:px-0 flex flex-col gap-5">
                        ...
                        <div className={`${offerBoxClass} w-full mt-2`}>
                             ...
                             {content.variants?.enabled && content.variants.showOnPage && (
                                <div className="mb-6 animate-in fade-in slide-in-from-top-2" id="variants-section">
                                    ...
                                    <div className="grid grid-cols-1 gap-2">
                                        {content.variants.options.map((variant) => (
                                            <div key={variant.id} className="relative group/variant">
                                                <label 
                                                    className={`flex items-center justify-between p-3 border rounded-xl cursor-pointer transition-all relative ${selectedVariant === variant.id ? 'border-blue-600 bg-blue-50 ring-1 ring-blue-600' : 'border-slate-200 hover:border-slate-300 bg-white'}`}
                                                >
                                                    {variant.isMostPopular && (
                                                        <div className="absolute -top-2 -right-1 bg-blue-600 text-white text-[8px] font-black px-1.5 py-0.5 rounded uppercase tracking-wider shadow-sm z-10 animate-pulse">
                                                            {labels.mostPopular}
                                                        </div>
                                                    )}
                                                    <div className="flex items-center gap-3">
                                                ...
                                                {onUpdateContent && (
                                                    <div className="absolute -right-2 top-1/2 -translate-y-1/2 flex flex-col gap-1 opacity-0 group-hover/variant:opacity-100 transition-opacity z-20 translate-x-full pl-2">
                                                        <button 
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                                                                const newOptions = content.variants!.options.map(o => ({
                                                                    ...o,
                                                                    isMostPopular: o.id === variant.id ? !o.isMostPopular : o.isMostPopular
                                                                }));
                                                                onUpdateContent({
                                                                    ...content,
                                                                    variants: { ...content.variants!, options: newOptions }
                                                                });
                                                            }}
                                                            title="Imposta come la più scelta"
                                                            className={`p-1.5 rounded-full shadow-md transition-colors ${variant.isMostPopular ? 'bg-blue-600 text-white' : 'bg-white text-slate-400 hover:text-blue-600'}`}
                                                        >
                                                        ...
                                                        <button 
                                                            onClick={(e) => {
                                                                ...
                                                                onUpdateContent({
                                                                    ...content,
                                                                    variants: { ...content.variants!, defaultId: variant.id }
                                                                });
                                                            }}
                                                            title="Imposta come predefinita"
                                                            className={`p-1.5 rounded-full shadow-md transition-colors ${content.variants?.defaultId === variant.id ? 'bg-emerald-600 text-white' : 'bg-white text-slate-400 hover:text-emerald-600'}`}
                                                        >
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-3 px-4 md:hidden z-50 shadow-[0_-4px_20px_rgba(0,0,0,0.15)] safe-area-bottom pb-6">
                {variantError && (
                    <div className="bg-red-50 border border-red-200 text-red-600 p-2 rounded-lg text-[10px] font-bold flex items-center gap-2 animate-in fade-in slide-in-from-bottom-1 mb-2">
                        <AlertTriangle className="w-3 h-3 shrink-0" />
                        <span>{content.language === 'Italiano' || !content.language ? `Seleziona "${content.variants?.title || 'una variante'}"` : `Select "${content.variants?.title || 'a variant'}"`}</span>
                    </div>
                )}
                <div className="flex items-center gap-3">
...
const LandingPage: React.FC<LandingPageProps> = ({ content, thankYouSlug, onRedirect, onPurchase, siteConfig, onUpdateContent, onGoHome }) => {
    ...
    const handleOpenOrder = () => {
        // Check if variant is required but not selected (only if shown on page)
        if (content.variants?.enabled && content.variants.showOnPage && content.variants.required && !selectedVariant) {
            setVariantError(true);
            
            // Scroll to variants section if it exists
